...
const base=await (await fetch(B+'/shared/base.css')).text();
ok('barre du haut partagee', base.includes('.topbar-brand')&&base.includes('--topbar-h'));
ok('etat de salon partage', base.includes('.rs-dot[data-state="live"]'));
ok('echelles definies', base.includes('--s4:')&&base.includes('--r-md:')&&base.includes('--t-body:'));
ok('racine fluide', base.includes('html { font-size: clamp('));
ok('liens non soulignes', base.includes('text-decoration: none'));
const hostjs=await (await fetch(B+'/host/app.js')).text();
const pjs=await (await fetch(B+'/player/app.js')).text();
ok('un seul socket pendant la partie', pjs.includes('stopWatching')&&pjs.includes("data.screen === 'jeu' ? stopWatching")||pjs.includes("if (data.screen === 'jeu') stopWatching()"));
ok('pas d ecran de chargement a chaque resolution', pjs.includes('const first = !state.party'));
const hostrt=await (await fetch(B+'/host/app.js')).text();
ok('visibilite des sections forcee, pas basculee', hostjs.includes("Boolean(wrongStep || wrongMode)"));
ok('hauteur de barre mesuree', (await (await fetch(B+'/player/app.js')).text()).includes('--topbar-real'));
ok('carte de participant partagee', base.includes('.pitem-main')&&base.includes('.pitem-sub')&&base.includes('.pitem.done'));
ok('bandeau de coupure sous la barre', (await (await fetch(B+'/player/styles.css')).text()).includes('top: var(--topbar-h)'));
ok('panneau lateral partage', base.includes('.panel-header')&&base.includes('--panel-w')&&base.includes('.rejoin'));
ok('code et QR de secours dans le panneau joueurs', ph.includes('id="g-qr"')&&ph.includes('class="rejoin"'));
ok('code retire de la barre du haut', !/topbar-right[\s\S]{0,200}id="g-code"/.test(ph));
ok('compteurs descendus dans les panneaux', ph.includes('id="pl-kpi"')&&!ph.includes('live-counters'));
ok('progression en titre de playlist', /Playlist[\s\S]{0,160}id="game-counter"/.test(ph));
